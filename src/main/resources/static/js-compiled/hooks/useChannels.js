...
export const useChannels = () => {
  ...
  // Adicionar canais
  const addChannels = useCallback(newChannels => {
    setChannels(prev => {
  ...
  // Atualizar canal
  const updateChannel = useCallback((channelId, updatedData) => {
    setChannels(prev => {
      const updated = prev.map(channel => channel.id === channelId ? {
        ...channel,
        ...updatedData
      } : channel);
      setFilteredChannels(updated);
  ...
  // Filtrar canais
  const filterChannels = useCallback(query => {
    if (!query.trim()) {
      setFilteredChannels(channels);
      return;
    }
    const filtered = channels.filter(channel => channel.nome.toLowerCase().includes(query.toLowerCase()) || channel.nome_grupo?.toLowerCase().includes(query.toLowerCase()) || channel.id_canal?.toLowerCase().includes(query.toLowerCase()));
